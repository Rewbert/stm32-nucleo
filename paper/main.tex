...
When the Non-secure participant invokes \inlinehaskell{sg}, it serializes the function identifier followed by its accumulated arguments into a memory-backed \inlinehaskell{BFILE}.
It passes this buffer and a result buffer through the C Non-secure Callable gateway.
The gateway validates the caller-provided memory ranges before forwarding their contents to the Secure Haskell dispatcher.
The dispatcher deserializes the identifier, finds the corresponding handler, deserializes its arguments, and executes the Secure computation.
It serializes the result into a Secure-side buffer and copies the resulting bytes to the validated Non-secure result buffer.
The Non-secure implementation then deserializes those bytes into the result of \inlinehaskell{sg}.
...
\begin{figure}[ht]
  ...
  \caption{\todo{Caption: the Secure and Non-secure firmware images, each built from its generated MicroHasTEE C file and the MicroHs runtime on top of that domain's peripheral drivers, running on the shared hardware.}}
...
The design of the driver library is similar to MicroHasTEE, where the same peripheral driver is compiled twice, once for the Secure domain and once for the Non-secure domain.
\begin{figure}[ht]
  \centering
  \includegraphics[width=\columnwidth]{graphics/microhastee-driver-partition.drawio.pdf}
  \caption{\todo{Caption: the peripheral driver library split into its Secure and Non-secure builds; some operations become no-ops in the Non-secure build but remain exported to keep a unified API.}}
...
However, some functions become no-op's in the Non-secure domain, but are exported none the less to maintain a unified API regardless of the domain.

\todo{Compute how big the TCB actually is in this case.}
