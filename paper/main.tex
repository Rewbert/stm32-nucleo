...
Trusted Firmware-M (TF-M) is the reference implementation of the Platform Security Architecture's Secure Processing Environment for Armv8-M and Armv8.1-M systems~\cite{trustedfirmwarem}.
...
\paragraph{TF-M}
To jusitfy MicroHasTEE at all, we need to describe precisely how MicroHasTEE relates to TF-M.
While TrustZone-M by itself gives us ``nothing more'' than two address spaces and a fault if execution from one space reaches into the other, TF-M implements a full trusted execution environment.
TrustZone-M gives us the mechanisms by which we can build these richer environments, but is by itself not a trusted execution environment.
MicroHasTEE does not claim to be a trusted execution environment, instead adopts the more bare-bones model that TrustZone-M implements.
We can model the two execution domains and ensure that they remain separated, save for the principled gateway.
However, MicroHasTEE checks the separation statically at compile time.
MicroHasTEE also ensures that the peripheral attribution that the program assumes is correctly configured, and rejects ill-formed programs at compile time.
