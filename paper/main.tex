...
Concerning running functional languages on MCUs, but not necessarily in a trusted execution environment, a noteworthy priject is the \textit{lbm} interpreter.
lbm is a Lisp interpreter that has been written solely with MCUs being the intended execution environment.
It optimises for performance in a resource constrainted environment, and implements many features and extensions not usually rpesent in Lisp.
It has seen great adoption by the VESC community, where it is used to program motorcontrollers and to implement UIs.
