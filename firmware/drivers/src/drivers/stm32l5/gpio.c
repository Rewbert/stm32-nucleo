...
static void stm32l5_gpio_set_security(struct gpio_dev *dev, gpio_security_t security) {
#if HAL_SECURE
    stm32l5_gpio_backend_t *backend = (stm32l5_gpio_backend_t*) dev->backend;

    stm32l5_gpio_set_seccfgr(backend, security);
#endif
}
...
static uint8_t stm32l5_gpio_pin(struct gpio_dev *dev) {
    stm32l5_gpio_backend_t *backend = (stm32l5_gpio_backend_t*) dev->backend;
    return backend->pin;
}
...
static void stm32l5_gpio_port_write(struct gpio_port_dev *dev, struct gpio_dev **devs, gpio_level_t *levels, size_t n) {
    stm32l5_gpio_port_backend_t *backend = (stm32l5_gpio_port_backend_t*) dev->backend;
...
static void stm32l5_gpio_port_read(struct gpio_port_dev *dev, struct gpio_dev **devs, gpio_level_t *out, size_t n) {
    stm32l5_gpio_port_backend_t *backend = (stm32l5_gpio_port_backend_t*) dev->backend;
...
static const gpio_port_driver_api_t stm32l5_gpio_port_api = {
    .write = stm32l5_gpio_port_write,
    .read  = stm32l5_gpio_port_read,
};

void stm32l5_gpio_port_create(gpio_port_dev_t *dev,
                              GPIO_TypeDef *port,
                              stm32l5_gpio_port_backend_t *backend_storage) {
    backend_storage->gpio = port;

    dev->api     = &stm32l5_gpio_port_api;
    dev->backend = backend_storage;
}
