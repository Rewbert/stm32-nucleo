...
uint32_t isr_vector[VECTOR_SIZE_WORDS] __attribute__((section(".isr_vector"))) = {
    ...
    0, /* SecureFault — not wired up, will trap in default_handler */
    0,
    0,
    0,
    (uint32_t)&sv_handler,
    ...
    /* External IRQs (offset 16+) — only EXTI0-15 wired, rest are 0 (catch-all default_handler) */
...
void mem_handler(void)     { while(1); }
void bus_handler(void)     { while(1); }
void usage_handler(void)   { while(1); }
void sv_handler(void)      { while(1); }
void debug_handler(void)   { while(1); }
void pend_handler(void)    { while(1); }
