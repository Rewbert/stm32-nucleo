...
/* EXTI register layout and IRQ numbering are identical to STM32L5:
 * EXTI0_IRQn = 11, so irqn = pin + 11 holds for pins 0–15.
 * EXTICR, RTSR1, FTSR1, IMR1, FPR1, RPR1 field names are unchanged. */

static exti_callback_t exti_callbacks[16] = {0};

static inline void stm32u5_exti_route_pin(uint8_t pin, exti_port_t port) {
    uint8_t  cr    = pin >> 2;
...
static inline void stm32u5_exti_set_edge(uint8_t pin, exti_edge_t edge) {
    switch (edge) {
...
static void stm32u5_exti_init(struct exti_dev *dev, exti_config_t *config) {
    stm32u5_exti_backend_t *backend = (stm32u5_exti_backend_t *) dev->backend;
    backend->pin = config->pin;

    stm32u5_exti_route_pin(config->pin, config->port);
...
static void stm32u5_exti_enable(struct exti_dev *dev) {
    stm32u5_exti_backend_t *backend = (stm32u5_exti_backend_t *) dev->backend;
    EXTIx->IMR1 |= (1U << backend->pin);
}

static void stm32u5_exti_disable(struct exti_dev *dev) {
    stm32u5_exti_backend_t *backend = (stm32u5_exti_backend_t *) dev->backend;
    EXTIx->IMR1 &= ~(1U << backend->pin);
}
...
void stm32u5_exti_create(exti_dev_t *dev, stm32u5_exti_backend_t *backend_storage, uint8_t pin) {
    dev->api             = &stm32u5_exti_api;
